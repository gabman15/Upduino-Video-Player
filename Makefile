...
clean:
	...
	rm -rf *.o *.bin *.asc *.json *.cf $(LOG_FILE) && touch $(LOG_FILE)
...
synth: # Synthesize the design
	...
	yosys -m ghdl -p '$(GHDL) --std=08 $(PRIMARY); synth_ice40 -json $(PRIMARY).json' $(ERR_STREAM)
...
svg:
	...
	yosys -m ghdl -p "ghdl --std=08 $(PRIMARY); prep -top $(PRIMARY); write_json svg.json" $(ERR_STREAM)
